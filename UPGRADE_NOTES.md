...
- `job_lock_keyspace`: Seed for the group-mutex advisory lock (default: 1). Required for correctness: all workers on the same table must share the same stable signed 32-bit integer; use a unique value per broker/table to avoid advisory-lock collisions.
