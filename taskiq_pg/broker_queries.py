...
CREATE TABLE IF NOT EXISTS {{table_name}} (
    ...
    status VARCHAR(20) DEFAULT '{MessageStatus.QUEUED.value}' CHECK (status IN ('{MessageStatus.QUEUED.value}', '{MessageStatus.ACTIVE.value}', '{MessageStatus.COMPLETED.value}')),
...
# Reclaim rows whose lease went stale (worker presumed dead). One param: timeout secs.
...
UPDATE {{table_name}}
SET status = '{MessageStatus.QUEUED.value}', retry_count = retry_count + 1
FROM stuck_messages
